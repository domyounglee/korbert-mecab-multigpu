...
def mapping_answer_korquad(p_json, rep_p, answer_text, answer_start, answer_end=-1):
	...
	if answer_text != rep_p['text'][answer_start:answer_end]:
		logger.info('[mapping_answer_korquad error]\t%s\t%s' %
		            (answer_text, rep_p['text'][answer_start:answer_end]))
		return None
...
def read_squad_examples_and_do_lang(input_file, is_training):
  ...
  for entry in input_data:
    for paragraph in entry["paragraphs"]:
      ...
      paragraph_morp = do_lang(paragraph_text)
      p_json = json.loads(paragraph_morp)['sentences']
      rep_p = get_morphs(p_json)
      #print(rep_p["text"])
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    if is_training:
      ...
      if example.a_end_morp + 1 < len(example.p_morp_token):
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      if is_training:
        ...
        if (example.a_begin_morp < doc_start or example.a_end_morp < doc_start or example.a_begin_morp > doc_end or example.a_end_morp > doc_end):
          continue
...
def input_fn_builder(train_features, seq_length, is_training, drop_remainder):
  """Creates an `input_fn` closure to be passed to TPUEstimator."""

  all_input_ids = np.array(
      [f.input_ids for f in train_features], dtype='i8')
  all_input_mask = np.array(
      [f.input_mask for f in train_features], dtype='i8')
  all_segment_ids = np.array(
      [f.segment_ids for f in train_features], dtype='i8')
  all_unique_ids = np.array(
      [[f.unique_id] for f in train_features], dtype='i8')



  def parse_fn(elem0,elem1,elem2,elem3):

    SentenceBatch = {"input_ids": elem0,
                  "input_mask":elem1,
                  "segment_ids":elem2,
                  "unique_ids":elem3}
  ...
  def generator_fn(train_features):
    for f in train_features:
      yield (f.input_ids, f.input_mask, f.segment_ids, f.unique_id)
  ...
  def input_fn(params):
    ...
    #d = tf.data.TFRecordDataset(train_features)
    
    #d = tf.data.Dataset.from_tensor_slices(
    #	(all_input_ids, all_input_mask, all_segment_ids, all_unique_ids))
    d = tf.data.Dataset.from_generator(functools.partial(
    	generator_fn, train_features), output_types=(tf.int64, tf.int64, tf.int64, tf.int64), output_shapes=([seq_length],[seq_length],[seq_length],[]))
    if is_training:
      d = d.repeat()
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for pred in prelim_predictions:
      ...
      final_text = example.p_raw_bytes[p_begin_morp:p_end_morp].decode(
      ).strip()
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = read_squad_examples_and_do_lang(
  ...
  if FLAGS.do_predict:
    eval_examples = read_squad_examples_and_do_lang(
